...
**PostgreSQL is recommended for most production deployments**, but SQLite can be valid for certain use cases.

**Use PostgreSQL when:**
- Multiple servers or workers share the same promise store
- You need high availability and replication
- Your deployment serves multiple teams or projects
...
Resonate's server and SDK are lightweight enough to support micro deployments - you can run isolated server instances with SQLite for specific users or projects. For shared, multi-tenant deployments, PostgreSQL provides better concurrency and HA options.
...
1. **State preserved in PostgreSQL** - All promise state is persisted, survives restart
2. **Workers handle disconnection** - Workers detect server disconnect and retry connections
3. **Graceful shutdown available** - Server responds to signals and attempts graceful cleanup (configurable timeout: default 10s)
4. **Workers resume** - When server comes back, workers reconnect and continue from checkpoints

```bash title="Server restart"
# Stop the server (sends signal for graceful shutdown)
...
The `timeout` configuration option controls how long the server waits during graceful shutdown (default: 10s). See [Server configuration](/operate/run-server#customize-configuration) for details.
...
Resonate exposes metrics, logs, and traces for production monitoring. This section covers what to monitor; see [Observe deployments](/operate/observe) for the complete catalog of available metrics and logs.
