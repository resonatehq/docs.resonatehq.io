---
id: run-server
title: Set up and run the Resonate Server
sidebar_label: Run a Server
description: A single binary that can be deployed anywhere.
sidebar_position: 1
last_update:
  date: "09-16-2025"
tags:
...
Blah blah blah, this page will help you install and run the Resonate Server.
...
See the [resonate serve command](/operate/cli-reference/commands/serve) for a full list of available options that are available via the CLI.
...
Here is a `resonate.yml` file that explicitly sets every option to the server's build-in defaults.
Using this file is equivalent to starting Resonate with no configuration file.
See the [config key reference](#config-key-reference) for details on each option.
