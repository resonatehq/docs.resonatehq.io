...
```rust title="main.rs"
use resonate::Resonate;

let resonate = Resonate::new(resonate::ResonateConfig {
    token: Some(std::env::var("RESONATE_TOKEN").ok()),
    url: Some("http://localhost:8001".into()),
    ..Default::default()
})?;
```

The Rust SDK also reads `RESONATE_TOKEN` from the environment automatically.
...
- Review [server configuration](/deploy/run-server#cli-flags-reference) for all server flags
...
- [Server configuration](/deploy/run-server#cli-flags-reference) — Complete server flag reference
