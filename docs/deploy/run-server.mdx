...
```shell
Resonate Server starting port=8001
Operational config ...
```
...
Tags track the server release versions (e.g. `v0.9.3`) plus a rolling `latest`.

```shell title="Run the Resonate Server in Docker"
docker run --rm -p 8001:8001 -p 9090:9090 resonatehqio/resonate:v0.9.3
```
...
For a worked end-to-end tutorial, see [Deploy a Countdown Workflow to Google Cloud Run](/learn/deployments/google-cloud-run).
