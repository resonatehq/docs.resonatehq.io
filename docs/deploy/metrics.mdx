...
The metrics port defaults to `:9090`. The server does not use a YAML config file — configure it via CLI flags when starting the server (e.g., `resonate serve --metrics-addr :9090`). See `resonate serve --help` for all available flags.
