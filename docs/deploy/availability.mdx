...
The server does not expose a dedicated `/healthz` endpoint.
Use `GET /` which returns `405 Method Not Allowed` — this confirms the server is alive and accepting connections.

```bash title="Check server health"
curl -s -o /dev/null -w "%{http_code}" http://localhost:8001/
# Returns 405 when healthy
```
