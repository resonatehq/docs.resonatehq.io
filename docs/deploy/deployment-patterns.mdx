...
services:
  ...
  resonate-server:
    # No official image is published yet — build from source using a custom Dockerfile.
    # See /deploy/run-server#run-with-docker for the Dockerfile reference.
    build: ./resonate-server
    ports:
      - "8001:8001"
      - "9090:9090"
    environment:
      DATABASE_URL: postgres://resonate:secret@postgres:5432/resonate
    depends_on:
      postgres:
    ...
    spec:
      containers:
        - name: server
          # No official image is published yet — build from source using a custom Dockerfile.
          # See /deploy/run-server#run-with-docker for the Dockerfile reference.
          image: your-resonate-server:latest
          ports:
            - containerPort: 8001
              name: http
            - containerPort: 50051
