...
**Cause:**
Another process is using port 8001 (HTTP).
...
Pass `--server-port` to `resonate serve` to bind a different port:
```bash
...
1. **Validate TOML syntax** with any TOML linter, or paste it into [toml-lint.com](https://www.toml-lint.com/).

2. **Check section and field names** against [Run a server](/deploy/run-server#configuration). Sections are nested with dots: `[storage.postgres]`, not `[storage_postgres]`.
...
See [Security](/deploy/security) for the full auth setup.
...
**Solution:**
Use PostgreSQL for any deployment with concurrent writers:
