...
Pass `--http-addr` to `resonate serve` to bind to a different port:
```bash
resonate serve --http-addr :8003
```
...
curl -s -o /dev/null -w "%{http_code}" http://resonate-server:8001/

# Should return 405 (server is alive, endpoint is POST-only)
```
...
The server is configured via CLI flags — there is no YAML config file. Set the auth token when starting the server:
```bash
resonate serve --http-auth-token your-token
```

In the SDK, pass the matching token via `token:`:
```typescript
const resonate = new Resonate({
  url: "http://resonate-server:8001",
  token: "your-token",
