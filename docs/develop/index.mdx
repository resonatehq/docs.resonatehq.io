...
```typescript
const promise = resonate.promises.create();
```
...
```python
promise = resonate.promises.get("promise-id")
```
...
```typescript
const promise = resonate.promises.get("promise-id");
```
...
These are durable to durable APIs.
Used inside the "durable world" — a world that automatically resumes after recovery.
