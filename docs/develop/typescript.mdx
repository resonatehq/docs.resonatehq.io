...
1. Explicit parameter passed to constructor (highest priority)
...
Port of the Resonate Server.

Defaults to port "8001", the default port exposed by the Resonate Server HTTP API.

#### `RESONATE_URL`

A full URL of the Resonate Server (e.g http://localhost:8001).

If this one is set it takes precedence over `RESONATE_HOST` and `RESONATE_PORT`.

#### `RESONATE_SCHEME`

The scheme of RESONATE_URL, i.e. the thing behind "://".
Typically either 'https' or 'http'.
