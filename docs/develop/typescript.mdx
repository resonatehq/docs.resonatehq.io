...
```ts
const result = await resonate.rpc("invocation-id", "foo", ...args);

// optionally target a specific group
const result = await resonate.rpc("invocation-id", "foo", ...args, resonate.options({
  target: "workers",
}));
```
...
```ts
const handle = await resonate.beginRpc("invocation-id", "foo", ...args);
const result = await handle.result();
```
...
resonate.register("foo", function* (ctx: Context, ...args: any[]) {
    // ...
    const result = yield* ctx.rpc("bar", ...args);
    // do more stuff
    // ...
});
...
resonate.register("foo", function* (ctx: Context, ...args: any[]) {
    // ...
    const promise = yield* ctx.beginRpc("bar", ...args);
    // do more stuff
    const result = yield* promise;
    // ...
});
