---
id: typescript
title: Skill Guide | Resonate TypeScript SDK
...
See the [Quickstart guide] or [How to run a Resonate Server](/operate/run-server) for guidance on getting a server up and running.
...
A Resonate Client needs to be able to receive messages to know what to work on next.

HTTP Long Polling the Resonate Server directly is the default way a Resonate Client is configured to receives messages.
That is, if you do not specify specific message transport plugins, the Resonate Client will use the built-in Poller transport.

You do not have to do anything special to enable this transport.

```ts title="index.ts"
import { Resonate } from "@resonatehq/sdk";

const resonate = new Resonate({
  url: "https://localhost:8001", // HTTP Poller transport is used here by default
});
```

However, A Resonate Client can receive messages from many different transports, such as HTTP, Kafka, RabbitMQ, etc...

To configure different message transports, see [How to configure message transports](#).
