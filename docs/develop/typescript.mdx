...
➡️ [Resonate TypeScript SDK API reference](https://resonatehq.github.io/resonate-sdk-ts/index.html)
...
You can configure a default Durable Promise Timeout for all function executions in the [Application Node](/concepts/application-node) via top-level Resonate object:
...
If you want a function to retry in an attempt to [recover from an application-level](/concepts/failure-detection-and-recovery#application-level) issue, you need to either throw an error, or return a rejected promise.
...
You can configure a default Retry Policy for all function executions in the [Application Node](/concepts/application-node) via the top-level Resonate object:
