...
Options can be used with `.run()`, `.beginRun()`, `.rpc()`, and `.beginRpc()` as the final argument.

```ts
await resonate.run("invocation-id", foo, ...args, resonate.options({
...
resonate.register("foo", function* (ctx: Context, ...args: any[]) {
  // ...
  yield* ctx.run(bar, ...args, ctx.options({
    id: "custom-id",
    timeout: 60_000, // 1 minute in ms
    target: "poll://any@workers",
    tags: { key: "value" },
