...
Consider Resonate if:
- You want simpler primitives (functions, not service types)
- You prefer zero-dependency local development
- You want to use familiar async/await patterns
- You need flexible state management (your choice of DB)
- You want to avoid vendor-specific abstractions
- You value a gentler learning curve

## When to stick with Restate

Restate might be better if:
- You specifically need built-in K/V state per object
- Your team is already invested in Restate's service types
- You prefer batteries-included state management over flexibility
- You're already running Restate infrastructure in production
