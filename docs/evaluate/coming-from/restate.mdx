...
async function approveOrder(approvalId: string) {
  await resonate.promises.settle(approvalId, "resolved", { data: true });
}
```
...
// From anywhere (use the promise's generated ID)
await resonate.promises.settle(approvalPromise.id, "resolved", { data: "approved" });
```
