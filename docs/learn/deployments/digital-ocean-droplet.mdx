...
The examples below show how to target the public endpoints you just exposed and supply the same credentials you configured in the systemd unit flags.
...
# Python SDK v0.6.7 works with the legacy Resonate server only.
# Support for server v0.9.x is coming in a future release.
resonate = Resonate.remote(
    group="my-app-group",
    host="your-domain.com",
    store_port="443",
    message_source_port="443",
)
```
...
For TypeScript workers, set the `RESONATE_TOKEN` environment variable before starting the process so the client sends the same auth token configured in the systemd unit. If you exposed additional subsystems, repeat the pattern for each port.
