...
The Resonate server is configured entirely via CLI flags — there is no config file.
...
# Returns 405 when healthy (server is alive, endpoint is POST-only)
```
...
| ------------------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------ |
| `bind: address already in use`              | Another process owns the port    | Update the ports in the systemd unit `ExecStart` flags and Nginx                                 |
