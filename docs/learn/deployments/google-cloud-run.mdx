...
6. Expand **Container, Variables & Secrets → Container args** and add:
...
You validated HTTPS routing, port mapping, and Basic Auth on your Cloud Run service.

Next scaffold the countdown workflow that we will deploy as the serverless worker.
...
export const countdown = resonate.register(
  ...
  function* (ctx, total, delayMinutes, targetUrl) {
    for (let i = total; i > 0; i--) {
      yield* ctx.run(notify, targetUrl, `⏱️ Countdown: ${i}`);
      yield* ctx.sleep(delayMinutes * 60 * 1000);
    }
    yield* ctx.run(notify, targetUrl, "🚀 Liftoff!");
  }
);
...
You’ll pass the Resonate Server URL and Basic Auth credentials via environment variables.
...
- `--arg https://ntfy.sh/resonatehq-demo` — destination URL (replace with your ntfy topic or webhook).

Watch the ntfy topic (or logs) to see `Countdown: 5`, `Countdown: 4`, …, `🚀 Liftoff!` roll in. You can also inspect the workflow via `resonate promises get countdown-demo`.

This confirms the full loop: CLI → Resonate Server → Cloud Function → external webhook, with the workflow suspending itself between notifications without holding compute resources.
