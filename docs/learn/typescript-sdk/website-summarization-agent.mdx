...
function baz(_: Context, greetee: string): string {
  console.log("running baz");
  return `Hello ${greetee} from baz!`;
}

function bar(_: Context, greetee: string): string {
  console.log("running bar");
  return `Hello ${greetee} from bar!`;
}

export const foo = resonate.register(
  "foo",
  function* (ctx: Context, greetee: string): Generator<string, string, string> {
...
async function main() {
  try {
    const promiseId = "hello-world-example";
    const result = await foo.run(promiseId, "World");
    console.log(result);
  } catch (error) {
    console.error(error);
  }
...
Change the file name from `index.ts` to `worker.ts` and update it to reflect the following code, changing `foo()` to `downloadAndSummarize()`, `bar()` to `download()`, and `baz()` to `summarize()`.:
...
With these changes the worker now downloads the requested page and summarizes it, which sets up the rest of the tutorial.
...
In the next part of the tutorial, you switch from using `.beginRun()` to using `.beginRpc()` and asynchronously invoke `downloadAndSummarize()` via an RPC (Remote Procedure Call).

## Asynchronous RPC

In this part of the tutorial, you will move the code that invokes `downloadAndSummarize()` to a separate process.

Currently the code that invokes `downloadAndSummarize()` lives in the same file and process as the workflow itself.
...
You will move the invocation of `downloadAndSummarize()` to a separate process and use Resonate's `.beginRpc()` method to invoke it.
The `.beginRpc()` method means "invoke this function over there".

Start by creating a new file `invoke.ts` and add the following code:
