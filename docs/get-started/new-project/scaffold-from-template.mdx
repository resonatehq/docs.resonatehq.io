...
title: Scaffold a new project with the CLI
sidebar_label: Scaffold with CLI
sidebar_position: 1
...
But, you can use the CLI independently of the server — That is, don't need to run the server to get started.
...
// Local promise store: great for iterating locally.
const localResonate = Resonate.local();

// Remote promise store: connects to a Resonate Server (defaults to http://localhost:8001).
const remoteResonate = Resonate.remote();
```
...
export const foo = resonate.register(
  "foo",
  function* (ctx: Context, params: { value: string }) {
    // Durable logic lives here.
...
```ts
// Invoke the registered function in the same process
const localHandle = await foo.beginRun("foo-run-1", { value: "hello" });
const localResult = await localHandle.result();

// Invoke the function on a remote worker group
const remoteHandle = await resonate.beginRpc(
  "foo-run-2",
  "foo",
  { value: "hello" },
  resonate.options({ target: "poll://any@worker" })
);
const remoteResult = await remoteHandle.result();
```
...
export const foo = resonate.register("foo", function* (ctx: Context) {
  const connection = ctx.getDependency<Database>("db");
  // Use the shared dependency inside the workflow.
