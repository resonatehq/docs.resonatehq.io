...
The Resonate server (v0.9.3+) is configured entirely via CLI flags — there is no `resonate.yaml` or `resonate.yml` config file. Pass all options directly to `resonate serve`:

```shell title="Example: serve with auth and PostgreSQL"
resonate serve \
