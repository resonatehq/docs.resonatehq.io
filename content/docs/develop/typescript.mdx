...
pageTitle: Skill Guide | TypeScript SDK
...
This page reflects `@resonatehq/sdk` v0.10.0 (current on npm). APIs are subject to change in future releases.
</Callout>
...
<Callout type="tip" title="No URL means local mode (zero-dependency development)">
...
Resonate's `.promises.settle()` method settles a promise by ID into a terminal state.
The `state` argument is a discriminator that determines whether the promise is resolved, rejected, or canceled.
...
```ts
await resonate.promises.settle(
  id: string,
...
await resonate.promises.settle("promise-id", "resolved");
await resonate.promises.settle("promise-id", "resolved", {
  data: JSON.stringify({ approved: true }),
});

// Reject a promise (optionally with data describing the failure)
await resonate.promises.settle("promise-id", "rejected");
await resonate.promises.settle("promise-id", "rejected", {
  data: JSON.stringify({ reason: "denied" }),
});

// Cancel a promise
await resonate.promises.settle("promise-id", "rejected_canceled");
```

The three `state` values correspond directly to the promise terminal states — `resolved`, `rejected`, and `rejected_canceled`.
Canceling a promise is just a `rejected_canceled` settle; there is no separate `cancel` method.
...
It works well in conjunction with the [`.promises.settle()`](#promisessettle) method.
...
yield* ctx.sleep({ until: new Date() });
```
