...
Native `kafka://` transport is specified in the [Message Passing Protocol](https://distributed-async-await.io/spec/execution-model/message-passing#address-schemes) and on the roadmap. This page will gain a transport-plugin section once it ships.
</Callout>
...
It works well in conjunction with the [`.promises.resolve()`](#promisesresolve), [`.promises.reject()`](#promisesreject), and [`.promises.cancel()`](#promisescancel) methods.
