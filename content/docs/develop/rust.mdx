...
pageTitle: Skill Guide | Rust SDK
...
<Callout type="note" title="Client-side builders (`resonate.run()`, `resonate.rpc()`) also support `.version(u32)` and `.tags(HashMap<String, String>)`. These are not available on Context builders.">
