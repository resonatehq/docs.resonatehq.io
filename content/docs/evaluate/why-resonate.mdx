...
**Formal modeling.** The [Distributed Async Await specification](https://distributed-async-await.io/spec) and the [Async RPC protocol](https://www.async-rpc.io) underlying Resonate are formally modeled and continuously probed for correctness. The protocol is a contract, not a vibe.
...
The [Async RPC protocol](https://www.async-rpc.io), the [Distributed Async Await specification](https://distributed-async-await.io/spec), and the open-source Resonate server are all fully open. The promise, schedule, and task state transitions are formalised as an [executable Lean 4 abstract machine](https://github.com/resonatehq/resonate-specification) — the machine-checkable ground truth alongside the prose spec. Adopting Resonate means adopting a protocol, not a vendor. Everything you need to fork or build your own is available to you.
