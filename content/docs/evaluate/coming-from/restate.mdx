...
This page reflects `@resonatehq/sdk` v0.10.0 (current on npm) and `resonate-sdk` v0.6.7 for Python.
</Callout>
...
async function approveOrder(orderId: string) {
  await resonate.promises.settle(`${orderId}.0`, "resolved", { data: JSON.stringify(true) });
}
```

No special workflow service type, no separate handler methods - just functions and promises.

<Callout type="tip" title="Durable Promise IDs">
...
await resonate.promises.settle(`${orderId}.0`, "resolved", { data: JSON.stringify("approved") });
```
