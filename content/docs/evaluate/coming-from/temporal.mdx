...
<Callout type="tip" title="Want the migration code?">
For side-by-side migration code — canonical Temporal samples mapped to their
Resonate equivalents, one pattern at a time, in every SDK — see
[Migrate from Temporal](/evaluate/coming-from/temporal-migration).
</Callout>
