...
`temporalio/samples-{typescript,python,go}` repos (trimmed for focus, elisions
marked). Resonate blocks come from the [examples
library](https://github.com/resonatehq-examples), pinned to the latest released
SDKs. Temporal's Rust SDK is in Public Preview with no samples repo yet, so Rust
targets migrate from the Temporal **TypeScript** sample. Some patterns don't have
a Resonate example in every language yet; those gaps are noted inline.
</Callout>
...
**Source: Temporal TypeScript → target: Resonate Rust** (Temporal's Rust SDK has no samples repo yet).
...
Temporal's Rust SDK has no samples repo yet — its signal idiom is the TypeScript
one above. The Resonate Rust equivalent is in the tabs below.
...
**Source: Temporal TypeScript → target: Resonate Rust.** See the Temporal TypeScript `sleep-for-days` excerpt above.
...
**Source: Temporal TypeScript → target: Resonate Rust.** See the Temporal TypeScript excerpt above.
...
**Source: Temporal TypeScript → target: Resonate Rust.** See the Temporal TypeScript `saga` excerpt above.
...
`example-infinite-workflow-rs` does not exist, and Temporal's Rust SDK has no samples repo yet. The Resonate equivalent is a plain `loop`/`while` with `ctx.detached` for the unbounded case — treat as a known gap until an example ships.
</Callout>
