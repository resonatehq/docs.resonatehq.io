...
This tutorial was written and tested with Resonate Server v0.9.3 and Resonate TypeScript SDK v0.10.0.
...
<Callout type="tip" title="Logs reveal the replay">
...
1. Run a new Resonate Server (`resonate dev`) and make sure Ollama is running (`ollama run llama3.1`).
2. Run the worker: `bun run worker.ts`.
3. Run the gateway: `bun run gateway.ts`.
