...
- The **Resonate CLI** (`brew install resonatehq/tap/resonate`), more installation options are in the [Run a Server](/deploy/run-server) guide.
...
![Architecture diagram showing Resonate Server on Cloud Run communicating with Cloud Function](/images/google-cloud-run-deployment-arch.png)
...
<Callout type="tip" title="Auth in development">
...
![ntfy.sh web interface](/images/ntfy.sh-subscribe-to-topic.png)
...
Start a long-running countdown from your terminal using the Resonate CLI `invoke` command.
