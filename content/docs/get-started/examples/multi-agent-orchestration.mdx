...
The `crashOnWriter` argument is a demo lever — set it to `true` and the writer throws on its first attempt. Watch the logs: the researcher runs once, the writer retries until it succeeds, the reviewer runs once. Without checkpoints, the researcher would re-run on every writer retry.
...
Watch the logs; the researcher's "gathering findings" message only appears once across the retry sequence.
