...
The OpenAI client is injected into the workflow via `r.with_dependency(OpenAI(...))` and accessed inside each agent with `ctx.get_dependency(OpenAI)` — keeps the agent functions free of global imports and easy to test.
...
# ... surface approval_id externally (email, log, webhook) ...
