...
pageTitle: Async agent tools use case
description: Start a new project using the Claude async MCP tool server example.
keywords:
  - get-started
  - new-project
  - template
...
```python title="weather_data.py - MCP Server with Async Tools"
...
# Define the durable function
@resonate.register
def weather_data(ctx, latitude, longitude, year, month):
    # Long-running data gathering work
    # ...API calls, processing, etc...
    return response.json()

# Tool 1: Start background job, return promise ID
@mcp.tool()
def start_gathering(latitude, longitude, year, month):
    ...
    # Start durable execution, return immediately
    _ = weather_data.run(job_name, latitude, longitude, year, month)
    
    return {"job_name": job_name}

# Tool 2: Check if job is done (non-blocking)
@mcp.tool()
def probe_status(job_names):
    """Probe for the status of data gathering jobs."""
    statuses = []
    for job_name in job_names:
        handle = resonate.get(job_name)
        if handle.done():
            statuses.append({"job_name": job_name, "status": "complete"})
        else:
            statuses.append({"job_name": job_name, "status": "running"})
    return statuses

# Tool 3: Wait for result (blocking)
@mcp.tool()
def await_result(job_name):
