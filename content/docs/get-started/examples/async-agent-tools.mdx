...
<CloneRepoGrid>
  ...
  <CloneRepoCard
    sdk="Rust SDK"
    description="rmcp-based MCP server with a get_forecast tool backed by a durable Resonate workflow."
    link="https://github.com/resonatehq-examples/example-mcp-tools-rs"
  />
</CloneRepoGrid>
...
def probe_status(job_names):
    ...
    for job_name in job_names:
        statuses.append({"job_name": job_name, "status": "check /await_result for result"})
    return statuses


@mcp.tool()
def await_result(job_names):
    """Block until jobs complete and return their results."""
    async def _gather():
        results = {}
        for n in job_names:
            handle = resonate.run(n, weather_data)  # reconnects to existing
            results[n] = await handle.result()
        return results
...
```rust title="src/bin/weather_server.rs"
...
```shell
git clone https://github.com/resonatehq-examples/example-mcp-tools-rs
...
```shell title="Terminal 2 — durable worker"
cargo run --bin worker
```

The MCP server (`weather-server` binary) is launched by Claude Desktop directly via stdio — see the config below.
