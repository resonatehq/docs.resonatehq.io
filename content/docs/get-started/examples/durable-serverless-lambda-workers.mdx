...
resonate = Resonate(url=url, group="gateway")


def lambda_handler(event, context):
    if event["path"] == "/process-document" and event["httpMethod"] == "POST":
        job = json.loads(event["body"])
        # rpc dispatches to the worker group and returns immediately.
        resonate.options(target="poll://any@worker").rpc(
    ...
    if event["path"].startswith("/status/"):
        ...
        handle = resonate.options(target="poll://any@worker").rpc(
            f"doc/{job_id}", "process_document", {},
        )
        try:
            result = asyncio.run(asyncio.wait_for(handle.result(), timeout=0.1))
...
url = os.environ.get("RESONATE_URL", "http://localhost:8001")
resonate = Resonate(url=url, group="worker")


async def process_document(ctx: Context, job: dict):
    # Each ctx.run is a checkpoint. Crash mid-step, resume from the next.
    ...
    stored_at = await ctx.run(store_results, job, analysis)
...
resonate.register(process_document)
