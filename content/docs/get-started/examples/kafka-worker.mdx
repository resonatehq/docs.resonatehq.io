...
Both repos run against [Redpanda](https://redpanda.com/) for local development because the broker boots in seconds, but the code uses the Kafka wire protocol — `kafkajs` for TypeScript, `confluent-kafka-python` for Python. Point either at any Kafka-compatible broker — Apache Kafka, Redpanda, or a managed offering — by changing the bootstrap-server config.
</Callout>
...
`beginRun` / `begin_run` is non-blocking — the consumer keeps polling while the workflow runs in the background. The same `recordId` deduplicates retries by design.
