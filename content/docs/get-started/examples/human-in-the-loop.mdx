...
resonate = Resonate(url=url, group="workers")


async def send_email(ctx: Context, promise_id: str) -> None:
    print(f"workflow blocked, waiting on human interaction (promise id: {promise_id!r})")


async def foo(ctx: Context, workflow_id: str):
    # Latent durable promise — resolved externally.
...
def start_workflow():
    ...
    # Same workflow_id reconnects to a PENDING execution rather than starting a new one.
    handle = resonate.options(target="poll://any@workers").rpc(
...
def unblock_workflow():
    ...
    asyncio.run(resonate.promises.resolve(promise_id, Value(data="human_approval")))
