...
<CloneRepoGrid>
  ...
  <CloneRepoCard
    sdk="Python SDK"
    description="FastAPI webhook with the same validate → charge → receipt → ledger pipeline, exactly once."
    link="https://github.com/resonatehq-examples/example-webhook-handler-py"
  />
</CloneRepoGrid>
...
def process_payment(ctx: Context, event: WebhookEvent, simulate_crash: bool):
    ...
    # Update the accounting ledger.
    return (yield ctx.run(update_ledger, event, charge_id))
```
...
The Python repo runs in embedded mode (`Resonate.local()`) so no separate server is needed for the demo.

```shell title="Terminal 1 — webhook server"
