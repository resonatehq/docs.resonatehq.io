...
<CloneRepoGrid>
  ...
  <CloneRepoCard
    sdk="Rust SDK"
    description="A worker that sleeps for N seconds via ctx.sleep(Duration), dispatched via async RPC."
    link="https://github.com/resonatehq-examples/example-durable-sleep-rs"
  />
</CloneRepoGrid>
...
async def sleeping_workflow(ctx: Context, wf_id: str, secs: float):
    print(f"Workflow {wf_id} starting, will sleep for {secs} seconds.")
    await ctx.sleep(timedelta(seconds=secs))
    return f"Workflow {wf_id} completed after sleeping for {secs} seconds."


resonate.register(sleeping_workflow)
...
async def main() -> None:
    url = os.environ.get("RESONATE_URL", "http://localhost:8001")
    resonate = Resonate(url=url, group="client")
    try:
        handle = resonate.options(target="poll://any@worker").rpc(
            "sleep-workflow-1", "sleeping_workflow", "sleep-workflow-1", 5.0,
        )
