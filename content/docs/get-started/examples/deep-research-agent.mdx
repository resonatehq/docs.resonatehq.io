...
<CloneRepoGrid>
  ...
  <CloneRepoCard
    sdk="Python SDK"
    description="Recursive research agent with parallel subagents via async ctx.run."
    ...
    while True:
        ...
        if message.tool_calls:
            ...
            for tool_call in message.tool_calls:
                ...
                if tool_call.function.name == "research":
                    handle = ctx.run(research, args["topic"], depth - 1)
...
The recursion is the whole pattern. `ctx.beginRpc` (TS) / `ctx.run(research, ...)` without `await` (Python) / `ctx.run(...).spawn()` (Rust) dispatches a child `research` execution and returns a handle immediately — children can be awaited later, each checkpointing on its own LLM calls. The outer loop awaits handles in order and feeds their results back to the LLM as tool messages.
