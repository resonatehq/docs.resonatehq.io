...
<CloneRepoGrid>
  ...
  <CloneRepoCard
    sdk="Python SDK"
    description="Notification fan-out across email, SMS, Slack, and push using asyncio.gather."
...
Each branch becomes its own durable promise. Spawn them with `beginRun` (TypeScript), `asyncio.gather(ctx.run(...), ...)` (Python), `.spawn()` (Rust), or `ctx.RPC` (Go) — all branches run concurrently. If a single branch fails and retries, the others are already checkpointed; they don't re-execute. Crash recovery only re-runs whatever was in flight when the worker died.
...
async def notify_all(ctx: Context, event: dict, simulate_crash: bool):
    # Fan-out: run all 4 channels concurrently, each a durable child.
    # Fan-in: gather joins when all complete. If push retries, the others
    # stay checkpointed and do NOT re-send.
    email, sms, slack, push = await asyncio.gather(
        ctx.run(send_email, event),
        ctx.run(send_sms, event),
        ctx.run(send_slack, event),
        ctx.run(send_push, event, simulate_crash),
    )
...
In v0.7.0, parallel fan-out uses `asyncio.gather` with multiple `ctx.run` calls. Each `ctx.run` is a durable child checkpoint; if push retries, email/SMS/Slack stay checkpointed and do not re-execute.
